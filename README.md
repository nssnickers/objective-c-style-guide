...
Выделяйте каждый параметр в методе словом, описывающим этот параметр, при этом перед первым параметром должен быть, помимо этого, использован `with`. Правильно не относится к параметру, если логично перед ним поставить другой предлог, например:
```objc
...
```objc 
int count = array[i].count;   // Compilation error
int count = [array[i] count]; // Ok
...
константы предпочтительнее по сравнению со строковыми литералами и числами, прошитыми в коде. Они должны быть объявлены с ключевым словом `static`, если только не используется макрос и конструкция `#define`.
